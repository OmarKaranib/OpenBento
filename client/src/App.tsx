          ...
          function AppContent() {
            ...
            // For signed-in Supabase users we mirror the widget layout to the
            // `dashboards` table so it follows them across devices. The flow:
            //   1. On sign-in, GET /api/dashboard once. If a remote layout
            //      exists, replace local widgets with it.
            //   2. After every widget change (debounced 1.5s), POST /api/dashboard
            //      with a Bearer access token from the live Supabase session.
            // Guests are unaffected — localStorage stays the only source of
            // truth for them. If the network call fails we silently fall back
            // to localStorage so the dashboard never blocks on a flaky API.
            ...
            }, [isAuthenticated, user?.id, getSupabaseAccessToken]);
            ...
            }, [widgets, isAuthenticated, user?.id, hydrationStatus, getSupabaseAccessToken]);
