...
function DraggableChannel({
  ...
  const fallbackBg = { news: 'bg-blue-500', science: 'bg-purple-500', gaming: 'bg-green-500', finance: 'bg-amber-500' }[channel.iconType] ?? 'bg-cyan-500';
  ...
  return (
    ...
    >
      <div className="w-[3.2rem] h-[3.2rem] rounded-lg bg-slate-700 flex items-center justify-center relative overflow-hidden flex-shrink-0">
        {showLogo ? (
          <img src={logoUrl} alt={channel.name} className="w-full h-full object-cover rounded-lg"
        ...
        {isLive === true && <div className="live-badge absolute -top-1 -right-1 w-[1rem] h-[1rem] bg-red-500 rounded-full animate-pulse" />}
      </div>
      ...
      {showSaveButton && (
        <button onClick={handleSaveClick} onPointerDown={(e) => e.stopPropagation()}
          ...
          title={isSaved ? 'Remove from Personal Library' : 'Save to Personal Library'} data-testid={`save-channel-${channel.id}`}>
          <Star className={`w-[1.4rem] h-[1.4rem] ${isSaved ? 'fill-amber-400' : ''}`} />
        </button>
      )}
      {showSaveButton && (
        <button onClick={handleBlockClick} onPointerDown={(e) => e.stopPropagation()}
          ...
          title={isBlocked ? 'Unblock channel' : 'Block channel'} data-testid={`block-channel-${channel.id}`}>
...
export function WidgetSidebar({
  isOpen, onClose,
  onChannelClick, onTemplateClick,
  urlValue = '', onUrlChange, onUrlSubmit,
  activeWidgetId, onImageUpload,
  isAuthenticated = false, openLoginModal,
}: WidgetSidebarProps) {
  // ── Primary tab: Streams vs Widgets ──
  ...
  useEffect(() => {
    const handler = () => setPersonalLibrary(loadPersonalLibrary());
    window.addEventListener('personalLibraryUpdated', handler);
    return () => window.removeEventListener('personalLibraryUpdated', handler);
  }, []);

  useEffect(() => {
    const handler = () => setBlockedChannels(loadBlockedChannels());
    window.addEventListener('blockedChannelsUpdated', handler);
    return () => window.removeEventListener('blockedChannelsUpdated', handler);
  }, []);

  const saveToPersonalLibrary = useCallback((channel: TrendingChannel) => {
    if (!isAuthenticated) { openLoginModal?.('Sign Up Required: Please log in or sign up to save channels to your library.'); return; }
    setPersonalLibrary(prev => {
      if (prev.some(c => c.id === channel.id)) return prev;
      const updated = [...prev, { id: channel.id, name: channel.name, url: channel.url, iconType: channel.iconType, category: channel.category, platform: channel.platform, channelId: channel.channelId, videoId: channel.videoId, savedAt: Date.now() }];
  ...
  const removeFromPersonalLibrary = useCallback((id: string) => {
    setPersonalLibrary(prev => { const u = prev.filter(c => c.id !== id); savePersonalLibrary(u); window.dispatchEvent(new CustomEvent('personalLibraryUpdated')); return u; });
  }, []);
  ...
  const blockChannel = useCallback((channel: TrendingChannel) => {
    setBlockedChannels(prev => {
      if (prev.some(c => c.id === channel.id)) return prev;
      const updated = [...prev, { id: channel.id, name: channel.name, url: channel.url, iconType: channel.iconType, category: channel.category, platform: channel.platform, channelId: channel.channelId, videoId: channel.videoId, blockedAt: Date.now() }];
  ...
  const unblockChannel = useCallback((id: string) => {
    setBlockedChannels(prev => { const u = prev.filter(c => c.id !== id); saveBlockedChannels(u); window.dispatchEvent(new CustomEvent('blockedChannelsUpdated')); return u; });
  }, []);
  ...
  const filteredChannels = useMemo(() => {
    ...
    if (searchQuery.trim()) {
      ...
      f = f.filter(c => c.name.toLowerCase().includes(q) || c.category.toLowerCase().includes(q) || c.platform.toLowerCase().includes(q));
    }
    return [...f].sort((a, b) => {
  ...
  const filteredPersonalLibrary = useMemo(() => {
    ...
    const q = searchQuery.toLowerCase();
    return personalLibrary.filter(c => c.name.toLowerCase().includes(q) || c.category.toLowerCase().includes(q) || c.platform.toLowerCase().includes(q));
  ...
  return (
    <>
      {/* Backdrop */}
      <div
        className={`fixed inset-0 bg-black/50 backdrop-blur-sm z-[99] transition-opacity duration-300 ${isOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}
      ...
      >
        ...
        <div className="p-[1.6rem] pb-0 border-b border-slate-700/80 flex-shrink-0">
          ...
          <div className="flex items-center justify-between mb-[1.4rem]">
            ...
            <button onClick={onClose} className="p-[0.6rem] hover:bg-slate-800 slot-button transition-colors" data-testid="button-close-sidebar">
          ...
          >
            <button
              ...
              className={`
                relative flex-1 flex items-center justify-center gap-[0.55rem]
                py-[0.75rem] px-[1rem] rounded-[0.6rem]
          ...
          {/* URL input — only visible in Streams tab */}
          {activeTab === 'streams' && (
            <div className="pb-[1.4rem]">
              ...
              <div className="flex gap-[0.6rem]">
                <input
                  ...
                  onKeyPress={(e) => { if (e.key === 'Enter' && urlValue.trim()) { e.preventDefault(); onUrlSubmit?.(urlValue); } }}
        ...
        <div className="flex-1 overflow-y-auto">
              ...
              <div className="flex flex-wrap gap-[0.4rem]">
                {([
                  ...
                  <button
                    ...
                    className={`flex items-center gap-[0.4rem] px-[1rem] py-[0.5rem] rounded-full text-[1.1rem] font-medium transition-all ${activeCategory === id ? `${active} text-white` : 'bg-slate-800 text-slate-400 hover:bg-slate-700 hover:text-slate-200'}`}
                    data-testid={`category-${id}`}
                  >
                    <Icon className="w-[1.2rem] h-[1.2rem]" />
                    {label}
                    {id === 'personal' && personalLibrary.length > 0 && (
                      <span className="ml-[0.2rem] px-[0.5rem] py-[0.1rem] bg-amber-500/30 rounded-full text-[0.9rem]">{personalLibrary.length}</span>
                    )}
                    {id === 'blocked' && blockedChannels.length > 0 && (
                      <span className="ml-[0.2rem] px-[0.5rem] py-[0.1rem] bg-red-500/30 rounded-full text-[0.9rem]">{blockedChannels.length}</span>
                    )}
                  </button>
                ))}
              </div>
              ...
              {activeCategory !== 'personal' && activeCategory !== 'blocked' && (
                <div>
                  <div className="flex items-center justify-between mb-[1rem]">
                    ...
                    <button onClick={() => refetchLinks()} className="p-[0.6rem] hover:bg-slate-800 slot-button transition-colors" title="Refresh" data-testid="button-refresh-links">
                  ...
                  <div className="space-y-[0.8rem]">
                    {filteredChannels.map(ch => (
                      <DraggableChannel key={ch.id} channel={ch} onClick={() => onChannelClick?.(ch)}
                        isLive={liveStatuses[ch.id]?.isLive} showSaveButton isSaved={isInPersonalLibrary(ch.id)}
                        isBlocked={isChannelBlocked(ch.id)} onSave={() => saveToPersonalLibrary(ch)}
                        onRemove={() => removeFromPersonalLibrary(ch.id)} onBlock={() => blockChannel(ch)}
                        onUnblock={() => unblockChannel(ch.id)} />
                    ))}
                    {filteredChannels.length === 0 && <p className="text-[1.2rem] text-slate-500 text-center py-[2rem]">No streams found</p>}
                  </div>
                </div>
              )}
                  ...
                  {filteredPersonalLibrary.length > 0 ? (
                    <div className="space-y-[0.8rem]">
                      {filteredPersonalLibrary.map(ch => (
                        <DraggableChannel key={ch.id} channel={ch as TrendingChannel} onClick={() => onChannelClick?.(ch as TrendingChannel)}
                          isLive={liveStatuses[ch.id]?.isLive} showSaveButton isSaved onRemove={() => removeFromPersonalLibrary(ch.id)} />
                      ))}
                    </div>
                  ) : (
                  ...
                  {filteredBlockedChannels.length > 0 ? (
                    <div className="space-y-[0.8rem]">
                      {filteredBlockedChannels.map(ch => (
                        <DraggableChannel key={ch.id} channel={ch} onClick={() => onChannelClick?.(ch as TrendingChannel)}
                          isLive={false} showSaveButton isSaved={false} isBlocked onUnblock={() => unblockChannel(ch.id)} />
                      ))}
                    </div>
                  ) : (
                    <div className="text-center py-[3rem]">
          ...
          ════════════════════════════════════════════ */}
          {activeTab === 'widgets' && (
            <div className="p-[1.6rem] space-y-[2.4rem]">

              {/* Intro */}
              <div>
                <p className="text-[1.25rem] font-semibold text-slate-200 mb-[0.4rem]">Interactive Blocks</p>
                <p className="text-[1.1rem] text-slate-500">Click to add a block to your dashboard. Blocks auto-place on the grid.</p>
              </div>
                ...
                <div className="grid grid-cols-2 gap-[0.8rem]">
                  {availableWidgets.map((w) => (
                    <button
                      ...
                      className={`
                        group relative flex flex-col items-center justify-center gap-[0.8rem]
                        p-[1.4rem] rounded-xl border ${w.border} ${w.bg}
                        transition-all duration-200
                        hover:scale-[1.03] hover:shadow-lg ${w.glow}
                        active:scale-[0.98]
                        focus:outline-none focus-visible:ring-2 focus-visible:ring-cyan-500
                      `}
                      data-testid={`widget-library-${w.id}`}
                    >
                      <div className="w-[4rem] h-[4rem] rounded-xl bg-slate-800/70 border border-slate-700/50 flex items-center justify-center group-hover:border-slate-600 transition-colors">
                        {w.icon}
                      </div>
                      ...
                      <span className={`absolute top-[0.7rem] right-[0.7rem] text-[0.8rem] font-bold px-[0.55rem] py-[0.2rem] rounded-full border ${w.badge}`}>
                        ADD
                      </span>
                    </button>
                  ))}
