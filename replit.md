...
- **Widget Marketplace (`/widgets`):** Public catalog page that lists installable Custom Widgets from a Zod-validated JSON manifest at `client/public/marketplace/widgets.json`. Responsive card grid renders a sandboxed thumbnail iframe per widget with name/author/description/category/version metadata and an "Add to dashboard" CTA. The Add button navigates to `/?install=<encoded url>`; the dashboard then opens the existing Block Library Custom Widget modal pre-filled with that URL so the standard trust banner / sandboxed iframe flow runs unchanged. Linked from `/dev/widgets` and from the in-sidebar Custom Widget add modal. Sample widgets ship under `client/public/examples/widgets/{pomodoro, hello-world, click-counter, public-quote}/index.html`.
