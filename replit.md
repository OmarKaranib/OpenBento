...
**Widget Specifics:**
- **Video Widget:** Integrates YouTube, Twitch, and other embeddable URLs. Features custom TV-style controls (Mute/Unmute, Pause/Play, Refresh, Delete, Seek Mode) that appear on hover. Auto-detects YouTube video IDs and Twitch channels, using `window.location.hostname` for Twitch parent parameters. Supports permanent YouTube channel embeds for live streams. Includes a 60-second auto-refresh for YouTube Live IDs and live status badges for Kick channels with 5-minute polling.
