...
- **Sky & Ambient Pack:**
  ...
  - **Sun & Sky Position:** SVG sky-arc with a sun-glyph dot positioned by `arcFraction` between sunrise and sunset, plus golden-hour countdown and current moon phase glyph + illumination %. Location precedence: explicit `sunLat/sunLon` override → **Weather widget's last resolved location** (shared via `client/src/widgets/weather-location.ts` pub/sub so the Sun & Sky widget reuses the user's existing weather city without re-prompting for geolocation) → `sunCity` geocoded via `/api/weather` → browser geolocation → London fallback. All math is local — no API calls — and lives in `client/src/widgets/sky-helpers.ts`.
  - **Earth at Night:** Slowly auto-rotating CSS sphere (configurable seconds-per-rotation, 20–600s) backed by a bundled equirectangular night-lights raster (`client/public/earth/night-lights.svg` — schematic continent silhouettes + city-light dot clusters and city-glow gradients) so the asset is HTTP-cached and decoded once across all widget instances. A radial day-side glow is anchored to the current sub-solar longitude (computed from `computeSunTimes(now, 0, 0).subSolarLon`) so the lit/dark line tracks the real terminator.
  - **ISS Live Tracker:** Equirectangular world-map dot fed by `/api/iss` (server proxy to `https://api.wheretheiss.at/v1/satellites/25544`, 25s in-memory cache). Configurable refresh interval (15–300s). Optional reference city (geocoded via `/api/weather`) shows great-circle distance to the current ISS subpoint, an "Above {city} now" hint when within 2000 km, **and a "Passing over {city} in {N}m" countdown** computed by `/api/iss/pass` (server-side estimator that batches 19 future timestamps over the next ~95 minutes through wheretheiss.at's `/positions` SGP4 endpoint, finds the closest-approach time, and caches the result for 5 min).
...
- `GET /api/iss/pass?lat=&lon=`: Next-overhead-pass estimator for the ISS Live Tracker widget. Validates lat/lon, samples 19 future timestamps (every 5 min over ~95 min) via wheretheiss.at's `/positions` SGP4 endpoint, returns `{atTs, minDistanceKm, willPassOverhead}` for the closest approach. 5-minute LRU TTL cache keyed by rounded `(lat, lon)`.
